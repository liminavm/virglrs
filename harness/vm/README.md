...
does not; the fence path, for one, is outside the classic line, and transfers have their own
line, by door, with a second line saying how scattered the attached page lists were -- the
multiplier on every transfer, and the number a boot has that no replay does.
