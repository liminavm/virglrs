...
""",
    ...
    # RETIRED: 'a blob is attributed to whichever context holds its id now'. It sabotaged the guard
    # that compared a blob's exporter against the context now holding that id, and there is no
    # longer a guard to weaken: a resource holds a share of the exporter's storage, and the exports
    # it is looked up in live inside the context object, so a new occupant of the id is asked a
    # table that was never the old one's. The bug is unrepresentable rather than untested, which is
    # the outcome a structural fix is for -- `a_reused_context_id` still pins the behaviour.
    (
