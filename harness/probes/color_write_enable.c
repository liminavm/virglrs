...
//
// The `color-write-enable` group of src/venus/unserved.txt, as a program that names its command.
//
//   vkCmdSetColorWriteEnableEXT
//
// From VK_EXT_color_write_enable, which anv advertises and KosmicKrisp does not, so the positive
// control and the venus run are both on a Linux host. zink sends this command wherever the
// extension is on, so on anv it is the first command of every GL client's first frame.
//
// One fragment shader writes 0xab to attachment 0 and 0xcd to attachment 1, both R32_UINT and
// cleared to 0. Two renders, each into a fresh pair of images: the first switches attachment 1
// off, the second switches attachment 0 off. Each switch has a consequence of its own -- a dropped
// command leaves both attachments written, and a count cut short leaves the second switch at
// whatever the driver defaults it to.
//
...
//
//   cc -O1 -o color_write_enable color_write_enable.c -lvulkan && ./color_write_enable
//
...
#define W 8u
#define H 8u
#define A_VALUE 0xabu
#define B_VALUE 0xcdu
...
static const uint32_t FRAG[] = {
    ...
    0x0004002b,0x00000006,0x00000009,0x000000ab,0x0004003b,0x00000007,0x0000000a,0x00000003,
    0x0004002b,0x00000006,0x0000000b,0x000000cd,0x00050036,0x00000002,0x00000004,0x00000000,
...
int main(void) {
    ...
    VkApplicationInfo app = {
        ...
        .pApplicationName = "venus-color-write-enable-probe",
    ...
    const char *ext = "VK_EXT_color_write_enable";
    if (!has_ext(exts, en, ext)) {
        fatal("VK_EXT_color_write_enable", VK_ERROR_EXTENSION_NOT_PRESENT);
    }
    ...
    VkPhysicalDeviceColorWriteEnableFeaturesEXT fcw = {
    ...
    PFN_vkCmdSetColorWriteEnableEXT set_color_write =
        (PFN_vkCmdSetColorWriteEnableEXT)vkGetDeviceProcAddr(dev, "vkCmdSetColorWriteEnableEXT");
    if (!set_color_write) {
        fatal("vkCmdSetColorWriteEnableEXT did not resolve", VK_ERROR_EXTENSION_NOT_PRESENT);
    }
    ...
    VkDynamicState dynamic = VK_DYNAMIC_STATE_COLOR_WRITE_ENABLE_EXT;
    VkPipelineDynamicStateCreateInfo ds = {
        .sType = VK_STRUCTURE_TYPE_PIPELINE_DYNAMIC_STATE_CREATE_INFO,
        .dynamicStateCount = 1,
        .pDynamicStates = &dynamic,
    };
    VkFormat formats[2] = {VK_FORMAT_R32_UINT, VK_FORMAT_R32_UINT};
    ...
    // Two renders, two attachments each: targets 0-1 for the first, 2-3 for the second.
    struct target t[4];
    for (int k = 0; k < 4; k++) {
        t[k] = target(pd, dev);
    }
    struct readback out = readback(pd, dev, 4);
    memset(out.p, 0xee, 4 * W * H * sizeof(uint32_t));
    ...
    // The first render keeps attachment 0 and drops attachment 1; the second the other way.
    VkBool32 switches[2][2] = {{VK_TRUE, VK_FALSE}, {VK_FALSE, VK_TRUE}};
    for (int pass = 0; pass < 2; pass++) {
        VkRenderingAttachmentInfo att[2];
        for (int a = 0; a < 2; a++) {
            att[a] = (VkRenderingAttachmentInfo){
                .sType = VK_STRUCTURE_TYPE_RENDERING_ATTACHMENT_INFO,
                .imageView = t[pass * 2 + a].view,
        ...
        set_color_write(cb, 2, switches[pass]);
    ...
    if (r == VK_SUCCESS) {
        ...
        check(count(run[0], A_VALUE) == all, "first render: attachment 0 on, written");
        check(count(run[1], 0) == all, "first render: attachment 1 off, still clear");
        check(count(run[2], 0) == all, "second render: attachment 0 off, still clear");
        check(count(run[3], B_VALUE) == all, "second render: attachment 1 on, written");
        printf("  texels as expected: %u, %u, %u, %u of %u\n", count(run[0], A_VALUE),
               count(run[1], 0), count(run[2], 0), count(run[3], B_VALUE), all);
    }
