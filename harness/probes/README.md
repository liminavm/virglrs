...
| `cooperative_matrix.c` | `cooperative-matrix` | 1 | 6/6 on lavapipe, llvmpipe (LLVM 22.1.8), Vulkan 1.4; KosmicKrisp and anv lack the extension | 6/6 on lavapipe via QEMU, with the same four shapes as the host. No refusal (Fedora 44, 2026-09-26) |
| `mesh_shader.c` | `mesh-shader` | 3 | 4/4 on lavapipe, llvmpipe (LLVM 22.1.8), Vulkan 1.4; KosmicKrisp and anv lack the extension | 4/4 on lavapipe via QEMU. No refusal (Fedora 44, 2026-09-26) |
| `acceleration_structure.c` | `acceleration-structure` | 11 | 11/11 on lavapipe, llvmpipe (LLVM 22.1.8), Vulkan 1.4; KosmicKrisp and anv lack the extension. Lavapipe has no indirect builds, so `vkCmdBuildAccelerationStructuresIndirectKHR` is scored by `the_acceleration_structure_commands_hand_the_driver_what_the_guest_sent` only | 11/11 on lavapipe via QEMU. No refusal (Fedora 44, 2026-09-26) |
| `ray_tracing_pipeline.c` | `ray-tracing-pipeline`, `ray-tracing-maintenance1` | 8 | 7/7 on lavapipe, llvmpipe (LLVM 22.1.8), Vulkan 1.4; KosmicKrisp and anv lack the extension. Swapping the two handles in the binding table fails all three traces. `vkCmdSetRayTracingPipelineStackSizeKHR` is scored by the log only; `vkGetRayTracingCaptureReplayShaderGroupHandlesKHR` needs a feature lavapipe does not report, so `ray_tracing_groups_are_held_to_the_pipeline` scores it | 7/7 on lavapipe via QEMU. No refusal (Fedora 44, 2026-09-26) |
