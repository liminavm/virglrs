...
| `extended_dynamic_state3.c` | `extended-dynamic-state3` | 21 | 25/25 on anv, Intel Iris Plus (ICL GT2), which serves 19 of the 21; 6/6 on KosmicKrisp, Apple M1 Max, which serves 5; both Vulkan 1.4. Rasterization stream, sample locations enable and line rasterization mode are scored by the log only; extra overestimation size and advanced blend by no host | not yet run |
