...
which only the decoder mints. A handle is the same case: its field is private and its only
constructor is `unsafe`, so the decoder and the driver's Vulkan calls make them and nothing else
does. A handler passes on what the guest sent; it cannot build what the driver will follow.
